// Write a program which takes 0’s and 1’s as input string and accept that string which have 10 at only last position using the concept of DFA.
...
int main(){
  ...
  for(ll i=0; i<s.length(); i++){
    ...
    else if(current_state == B && s[i] == '1') current_state = B;
    else if(current_state == C && s[i] == '0') current_state = A;
    else if(current_state == C && s[i] == '1') current_state = B;

  }
  if(final_state == current_state) cout<<s<<" is accepted 10 only at last position."<<endl;
